...
public class MyBotModule extends DefaultBWListener {
	...
	public void checkGameLostConditionAndLeaveGame() {
		...
		for (Unit unit : MyBotModule.Broodwar.self().getUnits()) {
			if (unit.getType().isBuilding()) {
				...
				if (unit.getType().canAttack() || unit.getType().canProduce() || (unit.getType().isSpellcaster() && unit.getType() != UnitType.Terran_Medic) || unit.getType() == UnitType.Zerg_Larva || unit.getType() == UnitType.Zerg_Egg || unit.getType() == UnitType.Zerg_Lurker_Egg || unit.getType() == UnitType.Zerg_Cocoon) {
		...
		if (isGameLostConditionSatisfied) {
			...
			MyBotModule.Broodwar.drawTextScreen(250, 115, "I will leave game in " + (maxDurationForGameLostCondition - (MyBotModule.Broodwar.getFrameCount() - gameLostConditionSatisfiedFrame)) + " frames");
	...
	void checkTimeOutConditionAndLeaveGame() {
		if (Broodwar.getFrameCount() >= 10) {
			...
			// 현재 프레임에서의 소요시간 기록
			timeElapsedAtFrame[Broodwar.getFrameCount()] = System.currentTimeMillis() - timeStartedAtFrame[Broodwar.getFrameCount()];
			...
			Broodwar.drawTextScreen(370, 5, "(" + (int) (Broodwar.getFrameCount() / (23.8 * 60)) + ":" + (int) ((int) (Broodwar.getFrameCount() / 23.8) % 60) + ")");
			...
			/*
			 * // 각 프레임별 소요시간 기록을 화면에 표시 y = 100; for (int i = Broodwar.getFrameCount() - 9;
			 * i < Broodwar.getFrameCount(); ++i) { Broodwar.drawTextScreen(260, y,
			 * "["+i+"] : "+timeStartedAtFrame[i]+" "+timeElapsedAtFrame[i]+" ms"); y += 10;
			 * }
			 * 
			 * // 최대 많은 시간을 소요한 프레임을 화면에 표시 y = 220; int maxI = 0; for (int i = 0; i <
			 * Broodwar.getFrameCount(); ++i) { if (timeElapsedAtFrame[maxI] <
			 * timeElapsedAtFrame[i]) { maxI = i; } } Broodwar.drawTextScreen(260, y,
			 * "["+maxI+"] : "+timeStartedAtFrame[maxI]+" "+timeElapsedAtFrame[maxI]+" ms");
			 */
