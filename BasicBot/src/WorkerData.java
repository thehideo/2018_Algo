...
public class WorkerData {
	...
	public void workerDestroyed(Unit unit) {
		...
		for (int i = workers.size() - 1; i >= 0; i--) {
			...
			if (worker == null) {
				...
				if (worker.getType().isWorker() == false || worker.getPlayer() != MyBotModule.Broodwar.self() || worker.exists() == false || worker.getHitPoints() == 0) {
	...
	public void setWorkerJob(Unit unit, WorkerJob job, Unit jobUnit) {
		...
		/*
		 * if (job == Idle) { std::cout << "set worker " << unit.getID() << " job " <<
		 * workerJobMap[unit] << " . 4 (idle) " << std::endl; }
		 */
	...
	public void setWorkerJob(Unit unit, WorkerJob job, UnitType jobUnitType) {
		...
		// System.out.println("setWorkerJob "+ unit.getID() + " from " +
		// workerJobMap.get(unit.getID()).ordinal() + " to " + job.ordinal());
	...
	/// ResourceDepot 에 충분한 수(미네랄 덩이 수 * mineralAndMineralWorkerRatio ) 의 미네랄 일꾼이
	/// 배정되어있는가
	public boolean depotHasEnoughMineralWorkers(Unit depot) {
		...
		// 근처 미네랄 수가 8개 라면, 일꾼 8마리여도 좋지만, 12마리면 조금 더 채취가 빠르다. 16마리면 충분하다. 24마리면 너무 많은
		// 숫자이다.
	...
	/*
	 * BWAPI::Unit WorkerData::getMineralToMine(BWAPI::Unit worker) { if (!worker) {
	 * return null; }
	 * 
	 * // get the depot associated with this unit BWAPI::Unit depot =
	 * getWorkerDepot(worker); BWAPI::Unit mineral = null; double closestDist =
	 * 10000;
	 * 
	 * if (depot) { BOOST_FOREACH (BWAPI::Unit unit,
	 * MyBotModule.Broodwar.getAllUnits()) { if (unit.getType() ==
	 * BWAPI::UnitTypes::Resource_Mineral_Field && unit.getResources() > 0) { double
	 * dist = unit.getDistance(depot);
	 * 
