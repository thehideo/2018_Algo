	...
	void actionMain(Unit unit, GroupAbstract groupAbstract) {
		if (InformationManager.Instance().enemyRace == Race.Zerg) {
			...
			if (MyBotModule.Broodwar.getFrameCount() - beforeTime > 24 * 2) {
				...
				// for (Unit Protoss_Carrier :
				// MyVariable.getEnemyUnit(UnitType.Protoss_Carrier)) {
				// if (Protoss_Carrier.getShields() > 50 && unit.getDistance(Protoss_Carrier) <
				// 800 && unit.canUseTech(TechType.EMP_Shockwave, Protoss_Carrier)) {
				...
				// for (Unit Protoss_Dragoon :
				// MyVariable.getEnemyUnit(UnitType.Protoss_Dragoon)) {
				// if (Protoss_Dragoon.getShields() > 50 && unit.getDistance(Protoss_Dragoon) <
				// 800 && unit.canUseTech(TechType.EMP_Shockwave, Protoss_Dragoon)) {
