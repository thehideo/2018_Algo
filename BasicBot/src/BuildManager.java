...
public class BuildManager {
	...
	public void update() {
		...
		// jyh
		if (MyVariable.isInitialBuildOrderFinished == true && MyBotModule.Broodwar.getFrameCount() % 200 == 0) {
			buildQueue.clearAll();
			return;
		}
		...
		while (!buildQueue.isEmpty()) {
			...
			if (currentItem.seedLocation != TilePosition.None && currentItem.seedLocation != TilePosition.Invalid && currentItem.seedLocation != TilePosition.Unknown && currentItem.seedLocation.isValid()) {
	...
	private Position getSeedPositionFromSeedLocationStrategy(BuildOrderItem.SeedPositionStrategy seedLocationStrategy) {
		...
		case MainBaseBackYard:
			...
			if (tempBaseLocation != null && tempChokePoint != null) {
				...
				// std::cout << "vy : " << vy;
				d = Math.sqrt(vx * vx + vy * vy) * 0.5; // BaseLocation 와 ChokePoint 간 거리보다 조금 짧은 거리로 조정. BaseLocation가 있는 Region은 대부분
														// 직사각형 형태이기 때문
				// std::cout << "d : " << d;
				...
				// std::cout << "z";
				if (tempTilePosition.isValid() == false || MyBotModule.Broodwar.isBuildable(tempTilePosition.getX(), tempTilePosition.getY(), false) == false) {
					...
					if (!isDeadlockCase && requiredUnits.size() > 0) {
						...
						while (it.hasNext()) {
							...
							if (requiredUnitType != UnitType.None) {
								...
								// BasicBot 1.1 Patch Start ////////////////////////////////////////////////
								// Zerg_Mutalisk 나 Zerg_Scourge 를 만들려고하는데 Zerg_Greater_Spire 만 있는 경우 deadlock 으로
								// 판정하는 버그 수정

								// 만들려는 유닛이 Zerg_Mutalisk 이거나 Zerg_Scourge 이고, 선행 유닛이 Zerg_Spire 인 경우,
								// Zerg_Greater_Spire 가 있으면 dead lock 이 아니다
								if ((unitType == UnitType.Zerg_Mutalisk || unitType == UnitType.Zerg_Scourge) && requiredUnitType == UnitType.Zerg_Spire && MyBotModule.Broodwar.self().allUnitCount(UnitType.Zerg_Greater_Spire) > 0) {
