...
public class BuildOrderItem {
	...
	public enum SeedPositionStrategy {
		...
		SupplyDepotLocation, // 공간이 쫍아서 Supply는 짓는 곳을 따로 정한다.
	...
	/// @param priority : 0 = 가장 낮은 우선순위. 숫자가 클수록 더 높은 우선순위. 큐에 있는 아이템들 중에서 가장 높은
	/// 우선순위의 아이템 (우선순위가 높으면 먼저 큐에 넣은 것)이 먼저 생산 진행됨.
	/// @param blocking : true = 지금 이것을 생산할 수 없으면, 이것 생산 가능해질때까지 기다림. false = 지금 이것을
	/// 생산을 할 수 없으면 다음것 먼저 생산 실행.
	...
	/// 건설 위치를 seedPositionStrategy 를 이용해서 찾는다. 못찾을 경우, 다른 SeedPositionStrategy 로 계속
	/// 찾는다
